...
function SystemsList() {
  ...
  const data = ["One Vendor One Contract"];
  const systems = {
    options: data,
    getOptionLabel: (option) => option,
  };

  const searchedSystem = data?.filter((option) => {
    if (systemSelect === undefined) return true;
    return option.Label === systemSelect;
  });

  const handleSystemSelect = (value) => {
    if (value === "One Vendor One Contract") {
      navigate(`/files/ovoc-files`);
    }
  };
  return (
            ...
            <Table>
              <TableHead className="bg-blue-100">
                <TableRow>
                  ...
                  <TableCell className="w-2/12 font-bold">System</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {searchedSystem?.map((option, index) => (
                  <TableRow key={index}>
                    <TableCell className="w-2/12">
                      <IconButton
                        onClick={(event) => {
                          handleSystemSelect(option);
                        }}
                      >
                        <VisibilityIcon />
                      </IconButton>
                    </TableCell>
                    <TableCell className="items-center justify-between w-10/12">
                      {option}
                    </TableCell>
                  </TableRow>
                ))}
